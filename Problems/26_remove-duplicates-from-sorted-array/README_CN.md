...
### 一句话直击本质
通过双指针法遍历数组，将不重复的元素依次覆盖到数组前部，从而移除重复项。

### 综合思路
该问题主要有以下解法：
1. **双指针法（迭代）**：使用两个指针，一个慢指针 `slow` 用于记录不重复元素的最后位置，另一个快指针 `fast` 用于遍历整个数组。当 `fast` 指向的元素与 `slow` 指向的元素不同时，说明遇到了新的不重复元素，将其覆盖到 `slow` 的下一个位置。
2. **递归法**：虽然不常用，但可以通过递归的方式实现类似的逻辑，即递归地处理数组的子部分，直到处理完所有元素。

### 全量伪代码
以下是双指针法的伪代码：
```plaintext
函数 removeDuplicates(数组 nums):
    初始化 slow 为 0
