...
def test_read_ai_sections_handles_bold_markers(tmp_path):
    readme = """# 103. 二叉树的锯齿形层序遍历
...
"""
    ...
    path.write_text(readme, encoding="utf-8")
    sections = read_ai_sections(path)
    assert "初始化队列" in sections["pseudocode"]
    assert "$O(n)$" in sections["complexity"]
    assert "一句话直击本质" not in sections["pseudocode"]


def test_read_ai_sections_handles_heading_markers(tmp_path):
    readme = """# 173. 二叉搜索树迭代器
...
"""
    ...
    path.write_text(readme, encoding="utf-8")
    sections = read_ai_sections(path)
    assert "初始化一个空栈 stack" in sections["pseudocode"]
    assert "$O(h)$" in sections["complexity"]
    assert "一句话直击本质" not in sections["pseudocode"]


def test_read_ai_sections_extracts_pseudocode_without_complexity_section(tmp_path):
    readme = """# 999. 占位标题
...
"""
    ...
    path.write_text(readme, encoding="utf-8")
    sections = read_ai_sections(path)
    assert "初始化 x 为 0" in sections["pseudocode"]
    assert sections["complexity"] == ""
