...
def entry(list_no, chapter, section, pid, slug, order=0):
    return ProblemEntry(list_no, f"list{list_no}", pid, slug, slug, None, chapter, section, order)
...
def test_single_list_problem_keeps_lingshen_origin():
    ...
    assignments, unplaceable = assign(entries, solved, {}, never_called)
    assert unplaceable == []
...
def test_problem_id_comes_from_the_taxonomy_not_the_directory_prefix():
    ...
    assignments, _ = assign(entries, solved, {}, never_called)
...
def test_uncovered_problem_asks_leetcode_for_its_number():
    ...
    assignments, _ = assign(entries, solved, tags, fake_resolver)
...
def test_cross_list_tie_breaks_deterministically():
    ...
    solved = [SolvedProblem(11, "container", "11_container")]
    first, _ = assign(entries, solved, {}, never_called)
    second, _ = assign(list(reversed(entries)), solved, {}, never_called)
...
def test_uncovered_problem_lands_in_the_most_tag_similar_section():
    ...
    assignments, unplaceable = assign(entries, solved, tags, fake_resolver)
